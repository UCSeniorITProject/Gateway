...
exports.createPrescription = async (req, reply)  => {
  try {
    const prescription = await prescriptionRequests.createPrescription(req.body.prescription);
    if(prescription.errorMessage !== undefined && prescription.errorMessage !== ''){
      return reply
                .status(400)
                .send({msg: prescription.errorMessage});
    }
    return {prescription};
  } catch (err){
    throw boomify(err);
...
exports.patchPrescription = async (req, reply) => {
  try {
    const prescription = await prescriptionRequests.patchPrescription(req.params.id, req.body.prescription);
    return {prescription};
  } catch (err) {
    throw boomify(err);
  }
